...
bool rgb_matrix_indicators_kb(void) {
    ...
    if(get_highest_layer(layer_state) == 1) {	
		rgb_matrix_set_color(5, 34,139,34);	
	}
    if(get_highest_layer(layer_state) == 2) {	
		rgb_matrix_set_color(5, 30,144,255);	
	}
    if(get_highest_layer(layer_state) == 3) {	//处于 CPAS LOCK 状态
		rgb_matrix_set_color(5, 220,20,60);	//把序号52的灯变为红色（R:255, G:0, B:0）
	}
    return true;
}
#endif
